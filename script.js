...
function showSlides() {
    ...
    // Reset to first slide if reached the end
...
document.addEventListener("DOMContentLoaded", function() {
    // Start slideshow only if invitation has been opened
    if (!document.getElementById('startingPoint').style.opacity || 
        document.getElementById('startingPoint').style.opacity !== '0') {
        // Don't start slideshow yet, will be started by openParallax
    } else {
        showSlides();
    }
});